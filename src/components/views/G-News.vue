      ...
      <v-card class="bg-white rounded-lg pa-2 w-75 mx-auto elevation-4 text-subtitle-2" v-if="post.commentary || isObjectNotEmpty(post.content) ">
        ...
        <div class="mt-2 bg-grey-lighten-5 pb-2 cursor-pointer" v-if="post.content">
          <div v-if="post.content.article">
            <v-img :src="post.content.article.imageUrl" width="100%" @click="zoomImage(post.content.article.imageUrl)" />
            <h4 class="font-weight-bold mt-1 ml-2">{{post.content.article && post.content.article.title}} </h4>
          </div>

          <div v-else-if="post.content.media">
            <v-img v-if="post.content.media.imageUrl" :src="post.content.media.imageUrl" width="100%" @click="zoomImage(post.content.media.imageUrl)" />
          ...
          <div class="grid-container" v-else-if="post.content.multiImage">
            <div class="full-width">
              <v-img :src="post.content.multiImage.images[0].imageUrl" width="100%" @click="zoomImage(post.content.multiImage.images[0].imageUrl)" />
            </div>
            <div class="grid-row">
                <div class="grid-item" v-for="(image, index) in post.content.multiImage.images.slice(1)" :key="index">
                  <v-img :src="image.imageUrl" width="100%" @click="zoomImage(image.imageUrl)" />
                </div>
            </div>
          </div>

          <v-img v-else :src="GestimumERP" width="100%" height="200px" @click="zoomImage(GestimumERP)" />
